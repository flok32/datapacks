...
tag @e[type=minecraft:armor_stand,name=PortalMarker,sort=nearest,limit=1,distance=..7] add width_z_3

# Mark as x axis portal (show base plate and give orientation east (-90)
data modify entity @e[type=minecraft:armor_stand,sort=nearest,limit=1,distance=..7] NoBasePlate set value false
execute as @e[type=minecraft:armor_stand,name=PortalMarker,sort=nearest,limit=1,distance=..7] at @s run teleport @s ~ ~ ~ -90 0