...
$execute if entity @e[name=RegionMarker,scores={regions_region=$(region)},tag=overworld] if entity @e[name=RegionMarker,scores={regions_region=$(region)},tag=the_nether] run function regions:logic/set_region_survival_02 {region:$(region)}
